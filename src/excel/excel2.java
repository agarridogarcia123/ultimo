/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package excel;

import java.io.*;
import java.util.List;
import javax.swing.*;
import jxl.format.Colour;
import jxl.format.UnderlineStyle;
import jxl.write.*;
import jxl.write.biff.RowsExceededException;
import jxl.*;

public class excel2{

	    private File archivo;
	    private List<JTable> tabla2;
	    private List<String> nom_hoja;
	    private WritableCellFormat	fila ;
	    private WritableCellFormat	columna;

	    public excel2(List<JTable> tab, File ar) throws Exception {
	        this.archivo = ar;
	        this.tabla2 = tab;
	        if(tab.size()<0){
	            throw new Exception("ERROR");
	        }
	    }

	    public boolean export() {
	        try {
	            DataOutputStream out = new DataOutputStream(new FileOutputStream(archivo));
	            WritableWorkbook w = Workbook.createWorkbook(out);
	            w.createSheet("Archivo", 0);
	           
	            for (int index=0;index<tabla2.size();index++) {
	                JTable table=tabla2.get(index);
	              
	                  WritableSheet s = w.getSheet(0);
	                  
	                for (int i = 0; i < table.getColumnCount(); i++) {
	                    for (int j = 0; j < table.getRowCount(); j++) {
	                        Object objeto = table.getValueAt(j, i);
	                        
	                        createColumna(s,table.getColumnName(i),i);
	                        createFilas(s,i,j+1,String.valueOf(objeto));
	                       
	                    }
	                }
	            }
	            w.write();
	            w.close();
	            out.close();
	            return true;

	        } catch (IOException ex) {
	            ex.printStackTrace();
	        } catch (WriteException ex) {
	            ex.printStackTrace();
	        }
	        return false;
	    }
	    private void createColumna(WritableSheet sheet,String columna,int number_columna)throws WriteException {
			
			WritableFont times10pt = new WritableFont(WritableFont.TAHOMA, 14);//poñemos o tipo de letra
			// definimos el formato d ela celda
			WritableCellFormat	times = new WritableCellFormat(times10pt);
			
			WritableFont times10ptBoldUnderline = new WritableFont(WritableFont.TAHOMA, 11, WritableFont.BOLD, false,UnderlineStyle.SINGLE);
			this.columna = new WritableCellFormat(times10ptBoldUnderline);//tipografia da columna
			CellView cevell = new CellView();
			cevell.setSize(920);
			cevell.setDimension(70);
			cevell.setFormat(times);
			cevell.setFormat(this.columna);
			
			addColumna(sheet, number_columna, 0, columna,this.columna);//engadimos columnas, 0 é a primeira fila
		}
	    
	    private void createFilas(WritableSheet sheet,int number_columna,int filas,String name_filas)throws WriteException {
			
			WritableFont times10pt = new WritableFont(WritableFont.ARIAL, 12);//eliximos letra
			times10pt.setColour(Colour.GOLD);
			WritableCellFormat	times = new WritableCellFormat(times10pt);//formato da celda
			times.setBorder(Border.TOP, BorderLineStyle.MEDIUM, Colour.GOLD);
			WritableFont times10ptBoldUnderline = new WritableFont(WritableFont.ARIAL, 12, WritableFont.NO_BOLD, false,UnderlineStyle.NO_UNDERLINE);
			fila = new WritableCellFormat(times10ptBoldUnderline);
			CellView cevell = new CellView();
			cevell.setDimension(70);
			cevell.setFormat(times);
			cevell.setFormat(fila);
			addFilas(sheet, number_columna, filas, name_filas,fila);
		}
	   
	    
	   
	    private void addColumna(WritableSheet sheet, int column, int row, String s,WritableCellFormat format)throws RowsExceededException, WriteException {
			Label label;
			label = new Label(column, row, s, format);
			sheet.addCell(label);
		}
	    private void addFilas(WritableSheet sheet, int column, int row, String s,WritableCellFormat format)throws WriteException, RowsExceededException {
			Label label;
			label = new Label(column, row, s, format);
			sheet.addCell(label);
		}

	
}